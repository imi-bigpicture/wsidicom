...
- Opening a sparse image is now faster and holds less memory. `PerFrameFunctionalGroupsSequence` is read when a frame of the instance is read rather than when the instance is opened, and reading it no longer creates a pydicom `Sequence`. The `Dataset` no longer holds the sequence.
...
- `Extended Offset Table Lengths` was written as the distance from one frame to the next, eight bytes more than the frame itself, so a reader that trusts the stated lengths read past the end of every frame.
- A DICOM-TIFF dual file whose `TileOffsets` and `TileByteCounts` hold a different number of entries had the longer silently truncated to the shorter. Such a file is now read from the pixel data instead.
...
- Reading a frame by an index below the first frame a file holds returned the last frame of that file, as a negative index reads from the end of the frame index rather than being rejected. Such an index now raises `WsiDicomOutOfBoundsError`, as does one past the last frame, which raised `IndexError`. Neither is reachable through `WsiDicom.open`.
- A point index or annotation index of zero or less in an annotation instance - both lists are one-based in DICOM - became a negative index, so a polyline or polygon came back with the wrong points and no error at all, and a measurement given such an index was attached to an annotation that does not exist and was dropped. Both lists are now validated when read.
- Searching for instances over DICOMweb reported any error other than `400 Bad Request` as a `KeyError` naming the status code, hiding the `HTTPError` it came from, and a status code the standard library does not know, or an error carrying no response, raised `ValueError` or `AttributeError`. An error that cannot be worked around is now raised as the `HTTPError` it is.
- `WsiDicom.clear_cache` and `Source.clear_cache` emptied the frame caches without resetting the size they account for, so a cache that had filled up stayed at its limit with nothing in it and evicted every frame put in from then on, leaving the caches unable to hold anything for the rest of the process.
...
- Image data were identified in the shared frame caches by `id()`, which is reused once an object is collected, so an image data created after another had been collected could be served that other one's frames. Each image data is now given an id of its own that is never reused. This is not reachable through `WsiDicom.open`, but is from a `Source` that creates image data on demand.
- Reading a DICOM-TIFF dual file set Pillow's `Image.MAX_IMAGE_PIXELS` to `None` while parsing the tiff tags, disabling decompression bomb protection for every Pillow decode in the process rather than only for wsidicom's. The tags are now read from the image file directory alone, without opening the file as an image, so the limit is never changed.
- `Series Description` was written as `LT` and `Image Comments` as `LO`, the opposite of the value representations those attributes have, so a long series description was written over-length rather than truncated and image comments were cut to 64 characters where `LT` allows 10240.
- The scheme version of a code was both written and read as `CodeSchemeVersion`, which is not the keyword of a DICOM attribute, so `Coding Scheme Version` was never written and a code read from a dataset that states one came back without it.
