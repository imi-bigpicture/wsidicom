...
- `Pixel Spacing` was written and read by the metadata schema as the column spacing before the row spacing, the opposite of what DICOM states: the first value is the spacing between adjacent rows, which is down the image. A spacing that was not square came out with its axes swapped, and a file written this way was read back the same way round by wsidicom but the other way round by anything else. `WsiDataset` already wrote them in the stated order.
